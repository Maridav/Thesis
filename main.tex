...
\begin{thm}
Пусть $R_1\equiv r$ п.н., где $r>0$, шары берутся в $l_1$-норме, и $d\geq 2$. 
...
Теперь пусть $i\not\in J_0$. Значит, $$\|\Delta_i\|_\infty \geq c_2\|\Delta_i\|_1 > c_2r/2,$$ где $c_2$ -- контанта, зависящаятолько от нормы. Поэтому $i$ принадлежит одному из $2d$ множеств вида: $$J_m^+ := \{i\colon \Delta_i^{(m)} > c_2r/2\}, \quad J_m^- := \{i\colon \Delta_i^{(m)} < - c_2r/2\}.$$
...
Заметим, что если $\xi_i\in V_{\bar k, \kappa_i}$ и $\xi_j\in V_{\bar k, \kappa_j}$ для некоторых $i, j\in J_d^+$, то $$\|\sigma\xi_i - \sigma\xi_j\|_1~\leq~(d-1)A(a^dn^{-1})^{1/(d-1)}.$$ 
Поэтому по лемме $$|\xi_i^{(d)} - \xi_j^{(d)}| \not\in [(d-1)A(a^dn^{-1})^{1/(d-1)}, c_3].$$

{\it {\bf Замечание.} При достаточно больших $a, n$ этот интервал корректно определен.}

Исходя из этого, разобъем $[0,a]$ на $\lceil c_3^{-1}\rceil$ частей длины не более $c_3$, и заметим, что если $\xi_i^{(d)}, \xi_j^{(d)}$ лежат в одной части, то $|\xi_i^{(d)} - \xi_j^{(d)}| \leq (d-1)A(a^dn^{-1})^{1/(d-1)}$. Поэтому тогда $|\kappa_i - \kappa_j|\leq ad$. Отсюда получаем искомую оценку: $$N(\bar k, d, +)\leq ad \lceil c_3^{-1}\rceil =: ac_4.$$
...
\begin{multline*}
    ...
    \leq  \exp \left(\left(\dfrac{c_5d}{(d-1)A^{d-1}}n\log n + \dfrac{c_5}{(d-1)A^{d-1}}n\log a^d\right)(1+o(1))\right) \left(\dfrac{\lambda\vol_d(U)e}{n-c_6a^d}\right)^{n-c_6a^d} \leq \\
    \leq 
...
\end{multline*}{}
Здесь использовалось следующее равенство:
