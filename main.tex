...
Получены ({\it пока что :)} ) более точные оценки вероятности больших уклонений в случае $l_1$-нормы, а именно, что существует число $\beta >1$, для которого $$\PP[K_a \geq n] = \exp((-\beta\cdot n\log n + \beta\cdot dn\log a + O(n))), \ n\to \infty, a \to\infty.$$
...
\begin{proof}
Для удобства рассмотрим гомотетию, переводящую куб $[0, a]^d$ в куб $[0,1]^d$. Тогда радиусы уменьшатся в $a$ раз, их количество сохранится.
    Рассмотрим набор ячеек: 
    \begin{multline*}
        \left\{\prod\limits_{m=1}^{d-1}\left[\dfrac{a^{1/(d-1)}(k_m+1/4)}{(8rn)^{1/(d-1)}}, \dfrac{a^{1/(d-1)}(k_m+3/4)}{(8rn)^{1/(d-1)}}\right]\right\}
    \times \\
    \left\{\left[l\left(\dfrac{c_1a^{1/(d-1)}}{(rn)^{1/(d-1)}} +\dfrac{2r}{a}\right) ,  l\left(\dfrac{c_1a^{1/(d-1)}}{(rn)^{1/(d-1)}}+\dfrac{2r}{a}\right) + \dfrac{c_1a^{1/(d-1)}}{(rn)^{1/(d-1)}}  \right]\right\},
    \end{multline*}{}
    ...
    \begin{equation*}
        \dfrac{a}{4r}\left(\dfrac{c_1a^{1/(d-1)}}{(rn)^{1/(d-1)}}+\dfrac{2r}{a}\right) + \dfrac{c_1a^{1/(d-1)}}{(rn)^{1/(d-1)}} = \dfrac{c_1a^{d/(d-1)}}{4r(rn)^{1/(d-1)}} + \dfrac{c_1a^{1/(d-1)}}{(rn)^{1/(d-1)}} + \dfrac{1}{2} < 1
    \end{equation*}{}
    при достаточно больших $a$ и $n$.
    
    Заметим, что если центры шаров $B(\xi_i, R_1)$, $B(\xi_j, R_1)$ лежат в разных ``рядах'', то есть $\xi_i^{(d)}$ и $\xi_j^{(d)}$ лежат в разных выбранных нами интервалах, то эти шары не пересекаются, так как расстояние между ``рядами'' хотя бы $2R_1$. Если же центры лежат в одном ``ряду'', то в каждом из шаров есть точка, не покрытая другим шаром. Действительно, рассмотрим точку  $x_i = \xi_i + (0, \ldots, 0, R_1)$. Заметим, что для достаточно больших $n$ и $a$ эта точка действительно лежит в кубе $[0, 1]^d$. Итак, при $j \not= i$
    \begin{multline*}
        \|x_i - \xi_j\|_1 = 
        |\xi_i^{(d)} + R_1 - \xi_j^{(d)} | + \sum_{m=1}^{d-1}|\xi_i^{(m)} - \xi_j^{(m)}| \geq\\
        \geq R_1 - |\xi_i^{(d)} - \xi_j^{(d)} | + \sum_{m=1}^{d-1}|\xi_i^{(m)} - \xi_j^{(m)}| \geq
        R_1 - \dfrac{c_1a^{1/(d-1)}}{(rn)^{1/(d-1)}} + \dfrac{1/2\cdot a^{1/(d-1)}}{(8rn)^{1/(d-1)}} > R_1.
    \end{multline*}{}
    ...
    Следовательно, 
    \begin{multline*}
        \PP[K_a\geq n] \geq \PP[E] = \dfrac{(a^d\lambda)^n}{n!}e^{-a^d\lambda}\cdot n!\cdot \left(\left(\dfrac{1/2\cdot a^{1/(d-1)}}{(8rn)^{1/(d-1)}}\right)^{d-1}\cdot \dfrac{c_1a^{1/(d-1)}}{(rn)^{1/(d-1)}}\right)^n = \\
...
Для удобства рассмотрим гомотетию, переводящую куб $[0, a]^d$ в куб $[0,1]^d$. Тогда радиусы уменьшатся в $a$ раз, их количество сохранится.
    ...
    \begin{multline*}
        \left\{\prod\limits_{m=1}^{d-1}\left[\dfrac{a^{1/(d-1)}(k_m+1/4)}{(8rn)^{1/(d-1)}}, \dfrac{a^{1/(d-1)}(k_m+3/4)}{(8rn)^{1/(d-1)}}\right]\right\}
    \times \\
    \left\{\left[l\left(\dfrac{c_2a^{1 + 2/(d-1)}}{r(rn)^{2/(d-1)}} +\dfrac{2r}{a}\right) ,  l\left(\dfrac{c_2a^{1 + 2/(d-1)}}{r(rn)^{2/(d-1)}}+\dfrac{2r}{a}\right) + \dfrac{c_2a^{1 +2/(d-1)}}{r(rn)^{2/(d-1)}}  \right]\right\},
    \end{multline*}{}
    где $k_m \in \{0, \ldots, \lfloor (8rn/a)^{1/(d-1)}\rfloor-1\}$, $l\in \left\{0, \ldots, \lfloor a/4r\rfloor\right\}$, $c_2 = 2^{-(4+6/(d-1))}$. 
    
    Все эти ячейки лежат в кубе $[0,1]^d$. По первым $d-1$ координатам это очевидно, проверим по последней. Действительно, 
    \begin{equation*}
        \dfrac{a}{4r}\left(\dfrac{c_2a^{1 + 2/(d-1)}}{r(rn)^{2/(d-1)}}+\dfrac{2r}{a}\right) + \dfrac{c_2a^{1 +2/(d-1)}}{r(rn)^{2/(d-1)}} = \dfrac{c_2a^{2+ 2/(d-1)}}{4r^2(rn)^{2/(d-1)}} + \dfrac{c_2a^{1+2/(d-1)}}{r(rn)^{2/(d-1)}} + \dfrac{1}{2} < 1
    \end{equation*}{}
    при достаточно больших $a$ и $n$.
    ...
    Заметим, что если центры шаров $B(\xi_i, R_1)$, $B(\xi_j, R_1)$ лежат в разных ``рядах'', то есть $\xi_i^{(d)}$ и $\xi_j^{(d)}$ лежат в разных выбранных нами интервалах, то эти шары не пересекаются, так как расстояние между ``рядами'' хотя бы $2R_1$. Если же центры лежат в одном ``ряду'', то в каждом из шаров есть точка, не покрытая другим шаром. Действительно, рассмотрим точку  $x_i = \xi_i + (0, \ldots, 0, R_1)$. Заметим, что для достаточно больших $n$ и $a$ эта точка действительно лежит в кубе $[0, 1]^d$. Итак, при $j \not= i$
        ...
        \geq R_1^2 - \dfrac{2c_2a^{2/(d-1)}}{(rn)^{2/(d-1)}} + \left(\dfrac{1/2a^{1/(d-1)}}{(8rn)^{1/(d-1)}}\right)^2 \geq R_1^2.
    \end{multline*}{}
    ...
    \begin{multline*}
        ...
        \cdot \left(\left(\dfrac{1/2\cdot a^{1/(d-1)}}{(8rn)^{1/(d-1)}}\right)^{d-1}\cdot
        \dfrac{c_2a^{1+ 2/(d-1)}}{r(rn)^{2/(d-1)}}\right)^n = \\
        =\exp\left(dn\log a - a^d\lambda - \left(1+\dfrac{2}{d-1}\right)n\log n + \left(2+\dfrac{2}{d-1}\right) n\log a  + O(n)\right) = \\
